...
class CCmdStr {
 public:
  ...
  bool GetValue(const int inum, char *value,
                const int ilen = 0);  // 字符串，ilen缺省值为0。
...
class CDir {
 public:
  ...
  char m_ModifyTime
      [21];  // 文件最后一次被修改的时间，即stat结构体的st_mtime成员。
  char m_CreateTime[21];  // 文件生成的时间，即stat结构体的st_ctime成员。
  char m_AccessTime
      [21];  // 文件最后一次被访问的时间，即stat结构体的st_atime成员。
...
bool RENAME(const char *srcfilename, const char *dstfilename,
            const int times = 1);
...
bool FGETS(const FILE *fp, char *buffer, const int readsize,
           const char *endbz = 0);
...
class CLogFile {
 public:
  ...
  bool
      m_bBackup;  // 是否自动切换，日志文件大小超过m_MaxLogSize将自动切换，缺省启用。
  long m_MaxLogSize;  // 最大日志文件的大小，单位M，缺省100M。
  pthread_spinlock_t
      spin;  // 声明自旋锁，给日志文件类加锁是防止多线程操作日志文件时出现的线程安全问题
...
class CIniFile {
 public:
  ...
  // 获取参数的值。
  // fieldname：字段的标签名。
  // value：传入变量的地址，用于存放字段的值，支持bool、int、insigned
  // int、long、unsigned long、double和char[]。
  // 注意，当value参数的数据类型为char
  // []时，必须保证value的内存足够，否则可能发生内存溢出的问题，
